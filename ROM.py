...
class ROM:
    def __init__(self, mesh=None, stiffnessMatrix=None, rhs=None, out_dim=0):
        ...
        if mesh is not None:
            ...
            # Preallocation for intermediate computation result
            self.glob_stiff_grad_times_u = PETSc.Vec().createSeq(self.mesh.n_cells * self.mesh.n_eq)
    ...
    def get_autograd_fun(self):
        ...
        class AutogradROM(torch.autograd.Function):
            ...
            def backward(ctx, grad_output):
                ...
                self.stiffnessMatrix.glob_stiff_grad.mult(self.solution, self.glob_stiff_grad_times_u)
                glob_stiff_grad_times_u = torch.tensor(self.glob_stiff_grad_times_u.array,
